...
void usbd_driver::enable (bool enable) {
  if (enable) {
    RCC.APB1ENR1.B.USBF      = SET;
    RCC.APB1RSTR1.B.USBFSRST = SET;
    RCC.APB1RSTR1.B.USBFSRST = RESET;
    USB_FS.CNTR.R = 0u;
    // wait for ready
    delay_ticks (0x2000);
    USB_FS.CNTR.modify([](auto & r)->auto {
      r.B.CTRM = SET;
      r.B.RESETM = SET;
      r.B.ERRM = SET;
...
#endif // 0    
  if (connect) USB_FS.BCDR.B.DPPU = SET;
  else         USB_FS.BCDR.R      = 0u;
  return res;
}

void usbd_driver::setaddr (const uint8_t addr) {
  USB_FS.DADDR.modify([=](auto & r)-> auto {
    r.B.ADD = addr;
    r.B.EF  = SET;
    return r.R;
  });
...
void usbd_driver::poll () {
  ...
  if (_istr.B.CTR) {
    ...
    _ev = usbd_evt_wkup;
    USB_FS.CNTR.B.FSUSP = RESET;
    USB_FS.ISTR.B.WKUP  = RESET;
  } else if (_istr.B.SUSP) {
    _ev = usbd_evt_susp;
    USB_FS.CNTR.B.FSUSP = RESET;
    USB_FS.ISTR.B.SUSP  = RESET;
  } else if (_istr.B.ERR) {
    _ev = usbd_evt_error;
    USB_FS.ISTR.B.ERR   = RESET;
  } else {
    return;
  }
  evt_handler (_ev, _ep);
