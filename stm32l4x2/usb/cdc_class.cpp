...
#define WSTR2 (u"Cubic Splines Interpolator")
#define WSTR3 (u"00001")
#define WSTR4 (u"Clang test")
...
extern "C" void xmemmove (void * d, const void * s, __SIZE_TYPE__ l) {
  char * dst = reinterpret_cast<char*>(d);
  const char * src = reinterpret_cast<const char*>(s);
  for (unsigned n=0; n<l; n++) dst [n] = src [n];
}
...
usbd_respond cdc_class::if_ctl (usbd_ctlreq * req) {
  ...
  case USB_CDC_SET_LINE_CODING:
    xmemmove (&cdc_line, req->data, sizeof (cdc_line));
